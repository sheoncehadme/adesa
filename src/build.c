...
IDSTRING(rcsid, "$Id: build.c,v 1.47 2004/11/07 18:09:31 dave Exp $");
...
DECLARE_DO_FUN(build_showmobprogs);
...
DECLARE_DO_FUN(build_renamearea);

void mpedit         args((CHAR_DATA *ch, MPROG_DATA *mprg, int mptype, char *argument));
...
const struct cmd_type build_cmd_table[] = {
    ...
    {"showmobprogs", build_showmobprogs, POS_STANDING, 0, LOG_NORMAL},
    ...
    {"helps", build_helps, POS_STANDING, 0, LOG_NORMAL},
    ...
    {"setarea", build_setarea, POS_STANDING, 87, LOG_NORMAL},
    ...
    {"makearea", build_makearea, POS_STANDING, 88, LOG_NORMAL},
    {"addobject", build_addobject, POS_STANDING, 0, LOG_NORMAL},
    {"addmob", build_addmob, POS_STANDING, 0, LOG_NORMAL},
    {"addreset", build_addreset, POS_STANDING, 0, LOG_ALWAYS},
    ...
    {"mpedit", build_set_mpedit, POS_STANDING, 82, LOG_NORMAL},
    ...
    {"renamearea", build_renamearea, POS_STANDING, 90, LOG_ALWAYS},
    /*
     * End of list.
     */
...
/* teleport */
TELEPORT_DATA      *room_to_teleport(ROOM_INDEX_DATA *room);

/* teleport */
TELEPORT_DATA      *
room_to_teleport(ROOM_INDEX_DATA *room)
{
    TELEPORT_DATA      *tele;
...
void
build_interpret(CHAR_DATA *ch, char *argument)
{
    ...
    if ((!IS_NPC(ch) && IS_SET(ch->act, PLR_LOG))
        || fLogAll || build_cmd_table[cmd].log == LOG_ALWAYS || get_trust(ch) < 87) {
...
/* -S- Addition: */
void
build_commands(CHAR_DATA *ch, char *argument)
{
...
void
build_showmob(CHAR_DATA *ch, char *argument)
{
    char                buf[MAX_STRING_LENGTH];
    ...
    sendf(ch, "@@WName: @@y%s.    @@WClass: @@y%s.\n\r", pMob->player_name, tab_mob_class[pMob->class].text);
    sendf(ch, "@@WVnum: @@y%d.  @@WSex: @@y%s.    @@WLv: @@y%d.    @@WAlign: @@y%d.\n\r", pMob->vnum, pMob->sex == SEX_MALE ? "male" : pMob->sex == SEX_FEMALE ? "female" : "neutral", pMob->level, pMob->alignment);

    if (pMob->custom_xp != -1)
        sendf(ch, "@@WCustom XP: @@y%d.\n\r", pMob->custom_xp);

    if (pMob->custom_minlev != -1 || pMob->custom_maxlev != -1)
        sendf(ch, "@@WCustom Min Level: @@y%d.   @@WCustom Max Level: @@y%d.\n\r", pMob->custom_minlev, pMob->custom_maxlev);

    if (pMob->path != NULL && pMob->path[0] != '\0')
        sendf(ch, "@@WPath: @@g%s@@N\n\r", pMob->path);

    sendf(ch, "@@WModifiers: AC: @@y%d.  @@WHitroll: @@y%d.  @@WDamroll: @@y%d.  @@WHP: @@y%d.  @@WMana: @@y%d.\n\r",
        pMob->ac_mod, pMob->hr_mod, pMob->dr_mod, pMob->hp_mod, pMob->mana_mod);

    sendf(ch, "@@WMob Flags:@@y\n\r%s", show_values(tab_mob_flags, pMob->act, TRUE));
    sendf(ch, "@@WAffected by:@@y\n\r%s", show_values(tab_affected_by, pMob->affected_by, TRUE));
    sendf(ch, "@@WSkill_Flags:@@y %s\n\r", bit_table_lookup(tab_mob_skill, pMob->skills));
    sendf(ch, "@@WCast_Flags:@@y %s\n\r", bit_table_lookup(tab_mob_cast, pMob->cast));
    sendf(ch, "@@WDef_Flags:@@y %s\n\r", bit_table_lookup(tab_mob_def, pMob->def));
    sendf(ch, "@@WShort description: @@y%s.\n\r@@WLong  description: @@y%s",
        pMob->short_descr, pMob->long_descr[0] != '\0' ? pMob->long_descr : "(none).\n\r");

    if (pMob->spec_fun != 0)
        sendf(ch, "@@WMobile has spec fun: @@y%s\n\r", rev_spec_lookup((void *) pMob->spec_fun));

    if ((pShop = pMob->pShop) != 0) {
        is_shopkeeper = 1;
        strcpy(buf1, "@@WMobile is a shopkeeper, will buy @@y");
...
void
build_showobj(CHAR_DATA *ch, char *argument)
{
    char                buf[MAX_STRING_LENGTH];
    ...
    sprintf(buf, "@@WVnum: @@y%d.  @@WType: @@y%s.\n\r", obj->vnum, rev_table_lookup(tab_item_types, obj->item_type));
    strcat(buf1, buf);
    ...
    if (obj->newcost == TRUE) {
        sprintf(buf, "@@WAlternative item cost: @@y%d.\n\r", obj->cost);
        strcat(buf1, buf);
    }

    if (obj->rarity != 0) {
        sprintf(buf, "@@WRarity level: @@y1/%d.\n\r", obj->rarity);
        strcat(buf1, buf);
    }
    ...
    if (obj->obj_fun != NULL) {
        sprintf(buf, "@@WObject has objfun: @@y%s.@@g\n\r", rev_obj_fun_lookup((void *) obj->obj_fun));
        strcat(buf1, buf);
    }
...
void
build_showroom(CHAR_DATA *ch, char *argument)
{
    char                buf[MAX_STRING_LENGTH];
    char                buf1[MAX_STRING_LENGTH];
    char                arg1[MAX_INPUT_LENGTH];
    ...
    sprintf(buf, "@@WVnum: @@y%d.  @@WSector:\n\r @@y%s", location->vnum, show_values(tab_sector_types, location->sector_type, FALSE));
    strcat(buf1, buf);
            ...
            if ((pexit = location->exit[door]) != NULL) {
                if ((display & DISPLAY_FULLDOORS)) {
                    ...
                    sprintf(buf, "@@WExit: @@y%7s @@W: To @@y%5i %s\n\r", sDirs[door],
                        pexit->to_room != NULL ? pexit->to_room->vnum : 0, pexit->to_room != NULL ? pexit->to_room->name : "");
                    strcat(buf1, buf);
                    ...
                    sprintf(buf, "%s: (%s)  @@WTo: @@y(%i) %s, %s\n\r", sDirs[door],
                        ...
                        pexit->to_room != NULL ? pexit->to_room->name : "Unknown", bit_table_lookup(tab_door_types, pexit->exit_info)
                        );
                    strcat(buf1, buf);
                }
            }
    ...
    if ((display & DISPLAY_RESETS) && location->first_room_reset != NULL) {
        ...
        strcat(buf1, "@@WResets:\n\r");
        for (Pointer = location->first_room_reset; Pointer; cnt++) {
            sprintf(buf1 + strlen(buf1), "%2d) ", cnt);
    ...
    if (location->locker != NULL) {
        ...
        sendf(ch, "\n\r@@N@@WLocker:\n\r"
            ...
            "  @@gMax Items:          @@y%s.\n\r"
            "  @@gMax Weight:         @@y%s.\n\r",
            bit_table_lookup(tab_locker_types, location->locker->flags),
            location->locker->maxitem == 0 ? "infinite" : number_comma_r(location->locker->maxitem, cbuf1),
            location->locker->maxweight == 0 ? "infinite" : number_comma_r(location->locker->maxweight, cbuf2));

        buf1[0] = '\0';

        for (cnt = 0; cnt < MAX_ITEM; cnt++)
            if (location->locker->types[cnt])
                sprintf(buf1 + strlen(buf1), "%s, ", rev_table_lookup(tab_item_types, cnt));
        ...
        if (location->locker->valid)
            sendf(ch, "  @@gValid Item Types:   @@y%s.@@N\n\r\n\r", buf1);
        else
            sendf(ch, "  @@gInvalid Item Types: @@y%s.@@N\n\r\n\r", buf1);
    }

    if (strcmp(location->nocmd, ""))
        sendf(ch, "\n\rDisallowed commands: @@y%s@@N\n\r", location->nocmd);

    if (strcmp(location->nospell, ""))
        sendf(ch, "Disallowed spells: @@y%s@@N\n\r", location->nospell);

    return;
}

void
build_showresets(CHAR_DATA *ch, char *argument)
{
    char                buf1[MAX_STRING_LENGTH];
    ROOM_INDEX_DATA    *location;
    BUILD_DATA_LIST    *Pointer;
...
char               *
reset_to_text(BUILD_DATA_LIST **pList, int *pcount)
{
    char                buf[MAX_STRING_LENGTH];
    ...
    switch (pReset->command) {
        ...
        case 'E':
            pObj = get_obj_index(pReset->arg1);
            if (pObj)
                sprintf(buf, "  stray 'equip' reset: object [%d] %s, on %s.\n\r", pReset->arg1, pObj->name, tab_wear_loc[(pReset->arg3)].text);
            else
        ...
        case 'C':
            pObj = get_obj_index(pReset->arg1);
            if (pObj)
                sprintf(buf, "  stray 'corpse' reset: object [%d] %s.\n\r", pReset->arg1, pObj->name);
            else
                sprintf(buf, "  stray 'corpse' reset: object %d (unknown).\n\r", pReset->arg1);
            strcat(buf1, buf);
            break;
        ...
        case 'M':                /* Load mob */
            ...
            while (*pList) {
                ...
                if (pReset->command != 'G' && pReset->command != 'E' && pReset->command != 'C')
                    break;
                ...
                if (pReset->command == 'G') {
                    ...
                    pObj = get_obj_index(pReset->arg1);
                    if (pObj != NULL)
                        sprintf(buf, "   equiped with [%d] %s, on %s.\n\r", pObj->vnum, pObj->name, tab_wear_loc[(pReset->arg3)].text);
                    else
                        sprintf(buf, "[%d] unknown object equipped on %s.\n\r", pReset->arg1, tab_wear_loc[pReset->arg3].text);
                    strcat(buf1, buf);
                }
                else if (pReset->command == 'C') {
                    pObj = get_obj_index(pReset->arg1);
                    if (pObj != NULL)
                        sprintf(buf, "   corpse loads [%d] %s with 1 in %d chance.\n\r", pObj->vnum, pObj->name, pReset->arg3);
                    else
                        sprintf(buf, "   corpse load invalid obj vnum [%d].\n\r", pReset->arg1);
                    strcat(buf1, buf);
                }
            }
            break;
...
void
build_findmob(CHAR_DATA *ch, char *argument)
{
    char                buf[MAX_STRING_LENGTH];
    char                buf1[MAX_STRING_LENGTH];
    char                arg[MAX_INPUT_LENGTH];
...
void
build_findmobroom(CHAR_DATA *ch, char *argument)
{
    char                buf[MAX_STRING_LENGTH];
    char                buf1[MAX_STRING_LENGTH];
    char                arg[MAX_INPUT_LENGTH];
...
void
build_findobject(CHAR_DATA *ch, char *argument)
{
...
void
build_findroom(CHAR_DATA *ch, char *argument)
{
...
void
build_setmob(CHAR_DATA *ch, char *argument)
{
    char                arg1[MAX_INPUT_LENGTH];
    char                arg2[MAX_INPUT_LENGTH];
   ...
   ************************************************************/
    ...
    if (arg1[0] == '\0' || arg2[0] == '\0') {
        ...
        send_to_char("  hr_mod dr_mod ac_mod hp_mod mana_mod\n\r", ch);
    ...
    if (lvalue != 0) {
        if (!ok_to_use(ch, tab_mob_flags, lvalue))
            return;
        ...
        sendf(ch, "Mobile flag '%s' %s.\n\r", rev_table_lookup(tab_mob_flags, lvalue), IS_SET(pMob->act, lvalue) ? "enabled" : "disabled");
        area_modified(pArea);
        return;
    }
    ...
    if (lvalue != 0) {
        if (!ok_to_use(ch, tab_affected_by, lvalue))
            return;
        ...
        sendf(ch, "Mobile affect '%s' %s.\n\r", rev_table_lookup(tab_affected_by, lvalue), IS_SET(pMob->affected_by, lvalue) ? "enabled" : "disabled");
        area_modified(pArea);
        return;
    }
    ...
    if (!str_cmp(arg2, "skills")) {
        ...
        if (!ok_to_use(ch, tab_mob_skill, lvalue))
            return;
        ...
        sendf(ch, "Mobile skill '%s' %s.\n\r", rev_table_lookup(tab_mob_skill, lvalue), IS_SET(pMob->skills, lvalue) ? "enabled" : "disabled");
        area_modified(pArea);
        return;
    }
    ...
    if (!str_cmp(arg2, "cast")) {
        ...
        if (!ok_to_use(ch, tab_mob_cast, lvalue))
            return;
        ...
        sendf(ch, "Mobile cast flag '%s' %s.\n\r", rev_table_lookup(tab_mob_cast, lvalue), IS_SET(pMob->cast, lvalue) ? "enabled" : "disabled");
        area_modified(pArea);
        return;
    }
    ...
    if (!str_cmp(arg2, "def")) {
        ...
        if (!ok_to_use(ch, tab_mob_def, lvalue))
            return;
        ...
        sendf(ch, "Mobile defensive flag '%s' %s.\n\r", rev_table_lookup(tab_mob_def, lvalue), IS_SET(pMob->def, lvalue) ? "enabled" : "disabled");
        area_modified(pArea);
        return;
    }
    ...
    if (!str_cmp(arg2, "class")) {
        ...
        if (!ok_to_use(ch, tab_mob_class, lvalue))
            return;
    ...
    if (!str_cmp(arg2, "hp_mod")) {
        if (value < 0 || value > 2000000000) {
            send_to_char("hp_mod range is 0 to 2,000,000,000.\n\r", ch);
            return;
        }
        pMob->hp_mod = value;
        area_modified(pArea);
    ...
    if (!str_cmp(arg2, "mana_mod")) {
        if (value < 0 || value > 2000000000) {
            send_to_char("mana_mod range is 0 to 2,000,000,000.\n\r", ch);
            return;
        }
        pMob->mana_mod = value;
        area_modified(pArea);
    ...
    if (!str_cmp(arg2, "xp")) {
        if (value < -1 || (value > 1500000 && ch->level < MAX_LEVEL)) {
            send_to_char("Custom XP range is 0 to 1500000 or -1 to turn this feature off. Implementors can set above 1.5mil.\n\r", ch);
            return;
        }
        pMob->custom_xp = value;
        area_modified(pArea);
        return;
    }

    if (!str_cmp(arg2, "minlev")) {
        if (value < -1 || value > 120) {
            send_to_char("Min Lev range is 0 to 120 or -1 to turn this feature off.\n\r", ch);
            return;
        }
        pMob->custom_minlev = value;
        area_modified(pArea);
        return;
    }

    if (!str_cmp(arg2, "maxlev")) {
        if (value < -1 || value > 120) {
            send_to_char("Max Lev range is 0 to 120 or -1 to turn this feature off.\n\r", ch);
            return;
        }
        pMob->custom_maxlev = value;
        area_modified(pArea);
        return;
    }

    if (!str_cmp(arg2, "path")) {
    ...
    if (!str_cmp(arg2, "short")) {
        ...
        strip_set(pMob->short_descr, "\n\r");

        code = last_code_used(pMob->short_descr);

        if (code != 'N' && code != '\0') {
            sprintf(buf, "%s@@N", pMob->short_descr);
            if (pMob->short_descr) free_string(pMob->short_descr);
    ...
    if (!str_cmp(arg2, "shop")) {
        ...
        if (!str_prefix("trade", arg3)) {
            ...
            if (num < 0 || num > MAX_TRADE) {
                sendf(ch, "trade must be between 0 and %d\n\r", MAX_TRADE);
                return;
            }
            ...
            if (!ok_to_use(ch, tab_item_types, value))
                return;
...
void
nuke_exit_resets(ROOM_INDEX_DATA *pRoomIndex, int door)
{
...
void
build_setroom(CHAR_DATA *ch, char *argument)
{
    char                arg1[MAX_INPUT_LENGTH];
    char                arg2[MAX_INPUT_LENGTH];
    char                arg3[MAX_INPUT_LENGTH];
    ...
    if (value != 0) {
        if (!ok_to_use(ch, tab_room_flags, value))
            return;
    ...
    if (!str_cmp(arg1, "name")) {
        ...
        strip_set(location->name, "\n\r");

        code = last_code_used(location->name);

        if (code != 'N' && code != '\0') {
            sprintf(buf, "%s@@N", location->name);
            if (location->name) free_string(location->name);
            location->name = str_dup(buf);
        }
    ...
    if (!str_cmp(arg1, "nocmd") && ch->level == MAX_LEVEL) {
        if (location->nocmd)
            free_string(location->nocmd);

        if (!str_cmp(arg2, "clear"))
            location->nocmd = str_dup("");
        else
            location->nocmd = str_dup(arg2);
    ...
    if (!str_cmp(arg1, "nospell") && ch->level == MAX_LEVEL) {
        if (location->nospell)
            free_string(location->nospell);

        if (!str_cmp(arg2, "clear"))
            location->nospell = str_dup("");
        else
            location->nospell = str_dup(arg2);
    ...
    if (!str_cmp(arg1, "teleport")) {
        ...
        TELEPORT_DATA      *telecurrent;

        argument = one_argument(argument, arg2);
        strcpy(arg3, argument);

        if (arg2[0] == '\0') {    /* no parameters, show help */
            send_to_char
                ("syntax:\n\r  teleport list\n\r  teleport add\n\r  teleport del\n\r  teleport flag <nomob/nopc/nocharm/fighting>\n\r  teleport vnum <vnum of room to teleport to>\n\r  teleport wait <# of seconds to teleport>\n\r  teleport out <message>\n\r  teleport in <message>\n\r",
                ch);
            return;
        }
        ...
        if ((telecurrent = room_to_teleport(location)) == NULL) {
            send_to_char("A Teleporter isn't operating in this room.\n\r", ch);
            return;
        }
        ...
        if (!str_cmp(arg2, "list")) {
            ...
            sendf(ch,
                "Teleporter Properties for %d:\n\r  flags: %s\n\r         %s\n\r         %s\n\r         %s\n\r         %s\n\r         %s\n\r  vnum: %d.\n\r  time to wait: %d\n\r  out message: %s\n\r  in message: %s\n\r",
                location->vnum, (IS_SET(telecurrent->flags, TEL_NOMOB)) ? "@@rNOMOB    @@g(mobs don't go through, charmies OK)@@N" : "@@GNOMOB    @@g(mobs go through)@@N",
                (IS_SET(telecurrent->flags, TEL_NOPC)) ?      "@@rNOPC     @@g(players don't go through)@@N"                       : "@@GNOPC     @@g(players go through)@@N",
                (IS_SET(telecurrent->flags, TEL_NOCHARM)) ?   "@@rNOCHARM  @@g(charmed mobs don't go through)@@N"                  : "@@GNOCHARM  @@g(charmed mobs go through)@@N",
                (IS_SET(telecurrent->flags, TEL_FIGHTING)) ?  "@@rFIGHTING @@g(PC/NPCs go through even when fighting)@@N"          : "@@GFIGHTING @@g(PC/NPCs don't go through when fighting)@@N",
                (IS_SET(telecurrent->flags, TEL_CHARMWAIT)) ? "@@rCHARMWAIT@@g(charmies wait for master if master is fighting)@@N" : "@@GCHARMWAIT@@g(charmies go through regardless if master is fighting)@@N",
                (IS_SET(telecurrent->flags, TEL_SOLITARY)) ?  "@@rSOLITARY @@g(only one person at a time can use teleporter)@@N"   : "@@GSOLITARY @@g(everyone can go through at the same time)@@N",
                telecurrent->vnum,
                telecurrent->wait,
                telecurrent->out,
                telecurrent->in);
        ...
        if (!str_cmp(arg2, "del")) {
            /* Delete teleporter */
            UNLINK(telecurrent, first_tele, last_tele, next, prev);
            free_string(telecurrent->in);
            free_string(telecurrent->out);
            PUT_FREE(telecurrent, tele_free);
            location->tele = NULL;
            top_tele--;

            send_to_char("Teleporter deleted!\n\r", ch);
            return;
        }

        if (!str_cmp(arg2, "vnum")) {
            if (arg3[0] == '\0') {
                send_to_char("Must input a number.\n\r", ch);
                return;
            }
            if (!is_number(arg3)) {
        ...
        if (!str_cmp(arg2, "wait")) {
            ...
            telecurrent->wait = atoi(arg3);
            return;
        }
        if (!str_cmp(arg2, "out")) {
            build_strdup(&telecurrent->out, arg3, TRUE, ch);
            return;
        }
        if (!str_cmp(arg2, "in")) {
            build_strdup(&telecurrent->in, arg3, TRUE, ch);
            return;
        }

        if (!str_cmp(arg2, "flag")) {
            if (arg3[0] == '\0') {
                send_to_char("Valid options: teleport flag <nomob/nopc/nocharm/fighting>\n\r", ch);
                return;
            }
            if (!is_name(arg3, "nomob nopc nocharm fighting charmwait solitary")) {
                send_to_char("Valid options: teleport flag <nomob/nopc/nocharm/fighting>\n\r", ch);
                return;
            }

            if (!str_cmp(arg3, "nomob")) {
                if (IS_SET(telecurrent->flags, TEL_NOMOB))
                    REMOVE_BIT(telecurrent->flags, TEL_NOMOB);
                else
                    SET_BIT(telecurrent->flags, TEL_NOMOB);
                send_to_char("OK.  Teleport Flag toggled.\n\r", ch);
                area_modified(location->area);
                return;
            }
            if (!str_cmp(arg3, "nopc")) {
                if (IS_SET(telecurrent->flags, TEL_NOPC))
                    REMOVE_BIT(telecurrent->flags, TEL_NOPC);
                else
                    SET_BIT(telecurrent->flags, TEL_NOPC);
                send_to_char("OK.  Teleport Flag toggled.\n\r", ch);
                area_modified(location->area);
                return;
            }
            if (!str_cmp(arg3, "nocharm")) {
                if (IS_SET(telecurrent->flags, TEL_NOCHARM))
                    REMOVE_BIT(telecurrent->flags, TEL_NOCHARM);
                else
                    SET_BIT(telecurrent->flags, TEL_NOCHARM);
                send_to_char("OK.  Teleport Flag toggled.\n\r", ch);
                area_modified(location->area);
                return;
            }
            if (!str_cmp(arg3, "fighting")) {
                if (IS_SET(telecurrent->flags, TEL_FIGHTING))
                    REMOVE_BIT(telecurrent->flags, TEL_FIGHTING);
                else
                    SET_BIT(telecurrent->flags, TEL_FIGHTING);
                send_to_char("OK.  Teleport Flag toggled.\n\r", ch);
                area_modified(location->area);
                return;
            }
            if (!str_cmp(arg3, "charmwait")) {
                if (IS_SET(telecurrent->flags, TEL_CHARMWAIT))
                    REMOVE_BIT(telecurrent->flags, TEL_CHARMWAIT);
                else
                    SET_BIT(telecurrent->flags, TEL_CHARMWAIT);
                send_to_char("OK.  Teleport Flag toggled.\n\r", ch);
                area_modified(location->area);
                return;
            }
            if (!str_cmp(arg3, "solitary")) {
                if (IS_SET(telecurrent->flags, TEL_SOLITARY))
                    REMOVE_BIT(telecurrent->flags, TEL_SOLITARY);
                else
                    SET_BIT(telecurrent->flags, TEL_SOLITARY);
                send_to_char("OK.  Teleport Flag toggled.\n\r", ch);
                area_modified(location->area);
                return;
            }
        ...
        send_to_char("Invalid Teleporter Parms!\n\r", ch);
        return;
    ...
    if (!str_cmp(arg1, "door")) {
        ...
        if (!str_cmp(arg3, "clear")) {
            ...
            if (!build_canwrite(pDestRoom->area, ch, 0)) {
                send_to_char("You cannot change this exit.\n\r", ch);
                return;
            }
            ...
            if (str_cmp(arg4, "onesided") && pDestRoom != NULL) {
                ...
                if (pExit != NULL)
                    if (pExit->vnum == location->vnum) {
                        ...
                        send_to_char("Note: Door in exit room does not point to this door, so not deleted.\n\r", ch);
                else
                    send_to_char("Note: There is no corresponding exit in the room pointed to by this exit.\n\r", ch);
            }

            sendf(ch, "Door to %d cleared.\n\r", pExit->vnum);
            return;
        }
        ...
        if (!str_cmp(arg3, "to")) {
            ...
            if (pDestRoom != NULL && !build_canwrite(pDestRoom->area, ch, 0)) {
                send_to_char("You cannot change the destination of this exit.\n\r", ch);
                return;
            }
            ...
            if (str_cmp(arg5, "onesided")) {    /* If NOT onesided, do checks */
                ...
                if (pDestExit->to_room != location) {
                    sendf(ch, "The door in room %d going %s points to room [%d] %s.\n\r",
                        value, sDirs[RevDirs[door]], pDestExit->vnum, pDestExit->to_room != NULL ? pDestExit->to_room->name : "");
                }
            }
            pExit->vnum = value;
            pExit->to_room = pRoom;
        ...
        if (!str_cmp(arg3, "keyword")) {
            if (!str_cmp(arg4, "$edit")) {
                send_to_char("Disabled.\n\r", ch);
                return;
            }

            build_strdup(&pExit->keyword, arg4, TRUE, ch);
            strip_set(pExit->keyword, "\n\r");
            return;
        }
        ...
        if (!str_cmp(arg3, "type")) {
            ...
            if (!ok_to_use(ch, tab_door_types, value))
                return;
    ...
    if (!str_cmp(arg1, "locker")) {
        ...
        if (arg2[0] == '\0') {
            send_to_char("syntax: locker <create|flags|maxitem|maxweight|valid|invalid|delete> <value>\n\r", ch);
            return;
        }
        ...
        if (!str_cmp(arg2, "create")) {
            ...
            for (pList = location->first_room_reset; pList != NULL; pList = pList->next) {
                pReset = pList->data;
            ...
            if (pReset) {
                send_to_char("Lockers cannot co-exist with rooms that populate objects at this stage.\n\r", ch);
                return;
            }
        ...
        /* create doesn't require the presence of a locker, but the rest of the commands do */
        if ((locker = location->locker) == NULL) {
            send_to_char("There is no locker in this room.\n\r", ch);
            return;
        }

        if (!str_cmp(arg2, "delete")) {
            UNLINK(locker, first_locker, last_locker, next, prev);
            PUT_FREE(locker, locker_free);
            location->locker = NULL;
            area_modified(location->area);

            send_to_char("Locker deleted.\n\r", ch);
            return;
        }
        ...
        if (!str_cmp(arg2, "flags")) {
            ...
            if (arg3[0] == '\0' || (arg3[0] != '-' && arg3[0] != '+')) {
                send_to_char("syntax: locker flags <+flag|-flag>\n\r", ch);
                return;
            }
            ...
            type = table_lookup(tab_locker_types, arg3 + 1);

            if (type == 0) {
                send_to_char("Invalid flag. Valid flags are:\n\r\n\r", ch);
                table_printout(tab_locker_types, buf);
                send_to_char(buf, ch);
                return;
            }

            if (!ok_to_use(ch, tab_locker_types, type))
                return;

            if (remove) {
                REMOVE_BIT(locker->flags, type);
                send_to_char("Flag removed.\n\r", ch);
            }
            else {
                SET_BIT(locker->flags, type);
        ...
        if (!str_cmp(arg2, "maxitem")) {
            if (arg3[0] == '\0' || !is_number(arg3)) {
                send_to_char("syntax: locker maxitem <amount>\n\r", ch);
                return;
            }

            locker->maxitem = abs(atoi(arg3));
            sendf(ch, "@@N@@gMax item amount set to @@y%s@@g.@@N\n\r", locker->maxitem == 0 ? "infinite" : number_comma(locker->maxitem));
            area_modified(location->area);
            return;
        }
        ...
        if (!str_cmp(arg2, "maxweight")) {
            if (arg3[0] == '\0' || !is_number(arg3)) {
                send_to_char("syntax: locker maxweight <amount>\n\r", ch);
                return;
            }

            locker->maxweight = abs(atoi(arg3));
            sendf(ch, "@@N@@gMax weight amount set to @@y%s@@g.@@N\n\r", locker->maxweight == 0 ? "infinite" : number_comma(locker->maxweight));
            area_modified(location->area);
            return;
        }

        if (!str_cmp(arg2, "valid")) {
            bool remove = FALSE;
            int type = 0;
            int cnt;

            if (arg3[0] == '\0' || (arg3[0] != '-' && arg3[0] != '+')) {
                send_to_char("syntax: locker valid <+type|-type>\n\r", ch);
                return;
            }
            ...
            type = table_lookup(tab_item_types, arg3 + 1);

            if (type > 0) {
                if (!ok_to_use(ch, tab_item_types, type))
                    return;
                ...
                if (remove)
                    locker->types[type] = FALSE;
                else
                    locker->types[type] = TRUE;

                sendf(ch, "The item type '%s' is now %s the valid list.\n\r", rev_table_lookup(tab_item_types, type), remove ? "removed from" : "added to");
        ...
        if (!str_cmp(arg2, "invalid")) {
            bool remove = FALSE;
            int type = 0;
            int cnt;

            if (arg3[0] == '\0' || (arg3[0] != '-' && arg3[0] != '+')) {
                send_to_char("syntax: locker invalid <+type|-type>\n\r", ch);
                return;
            }
            ...
            if (type > 0) {
                ...
                sendf(ch, "The item type '%s' is now %s the invalid list.\n\r", rev_table_lookup(tab_item_types, type), remove ? "removed from" : "added to");
                area_modified(location->area);
            }
            else
                send_to_char("Unknown item type.\n\r", ch);
...
void
build_setobject(CHAR_DATA *ch, char *argument)
{
    char                arg1[MAX_INPUT_LENGTH];
    char                arg2[MAX_INPUT_LENGTH];
    char                arg3[MAX_INPUT_LENGTH];
    ...
    if (IS_SET(pObj->extra_flags, ITEM_CLAN_EQ) && get_trust(ch) != 90) {
    ...
    if (!str_cmp(arg2, "cost")) {
        if (value > 0) {
            pObj->cost = value;
            pObj->newcost = TRUE;
            sendf(ch, "Cost set to %d.\n\r", pObj->cost);
        }
        else {
            pObj->cost = 0;
            pObj->newcost = FALSE;
            sendf(ch, "Cost set to %d. Newcost removed.\n\r", pObj->cost);
        }
        return;
    }
    ...
    if (!str_cmp(arg2, "rarity")) {
        if (!IS_SET(pObj->extra_flags, ITEM_RARE)) {
            send_to_char("Can't set a rarity value for an item that isn't even rare!\n\r", ch);
            return;
        }

        if (value != 0) {
            pObj->rarity = value;
            sendf(ch, "Rarity level set to 1/%d chance.\n\r", pObj->rarity);
        }
        else {
            pObj->rarity = 0;
            sendf(ch, "Rarity level set to %d. Back to normal.\n\r", pObj->rarity);
        }
        return;
    }
    ...
    if (!str_cmp(arg2, "short")) {
        ...
        strip_set(pObj->short_descr, "\n\r");

        code = last_code_used(pObj->short_descr);

        if (code != 'N' && code != '\0') {
            sprintf(buf, "%s@@N", pObj->short_descr);
            if (pObj->short_descr) free_string(pObj->short_descr);
    ...
    if (!str_cmp(arg2, "aff")) {
        ...
        if (!ok_to_use(ch, tab_obj_aff, location))
            return;
...
void
build_dig(CHAR_DATA *ch, char *argument)
{
    char                arg1[MAX_INPUT_LENGTH];
    char                arg2[MAX_INPUT_LENGTH];
    char                arg3[MAX_INPUT_LENGTH];
    ...
    if (vnum < pArea->min_vnum || vnum > pArea->max_vnum) {
        sendf(ch, "Vnum must be between %d and %d.\n\r", pArea->min_vnum, pArea->max_vnum);
        return;
    }
...
void
build_stop(CHAR_DATA *ch, char *argument)
{
...
void
do_build(CHAR_DATA *ch, char *argument)
{
    ...
    build_help(ch, "bmotd");    /* motd for builders -S- */
...
void
build_addmob(CHAR_DATA *ch, char *argument)
{
    char                arg1[MAX_INPUT_LENGTH];
    char                arg2[MAX_INPUT_LENGTH];
    MOB_INDEX_DATA     *pMobIndex;
...
void
build_addobject(CHAR_DATA *ch, char *argument)
{
    char                arg1[MAX_INPUT_LENGTH];
    char                arg2[MAX_INPUT_LENGTH];
    OBJ_INDEX_DATA     *pObjIndex;
...
void
build_addreset(CHAR_DATA *ch, char *argument)
{
    RESET_DATA         *pReset;
    ...
    if (arg1[0] == '\0' || (arg2[0] == '\0' && strcmp(arg1, "message"))) {
        ...
        send_to_char("       corpse  <n.mob-vnum> <obj-vnum> <chance>\n\r", ch);
    ...
    if (!is_name(arg1, "mob obj equip message give door rand corpse")) {
        build_addreset(ch, "");
        return;
    }
    ...
    if (!str_cmp(arg1, "mob")) {
        vnum = is_number(arg2) ? atoi(arg2) : -1;
        if (vnum < 0 || vnum > MAX_VNUM) {
            sendf(ch, "Vnum must be between 0 and %d.\n\r", MAX_VNUM);
            return;
        }
    ...
    if (!str_cmp(arg1, "equip")) {
        ...
        if (!ok_to_use(ch, tab_wear_loc, num))
            return;
    ...
    if (!str_cmp(arg1, "corpse")) {
        ...
        if (num < 1 || num > 30000) {
            send_to_char("Chance is out of bounds.\n\r", ch);
            return;
        }
    ...
    if (!str_cmp(arg1, "door")) {
        ...
        if (!ok_to_use(ch, tab_door_states, num))
            return;
...
void
build_delreset(CHAR_DATA *ch, char *argument)
{
    RESET_DATA         *pReset;
    BUILD_DATA_LIST    *pList;
    ...
    if (pReset->command == 'M') {
        ...
        for (pList = pNextList; pList; pList = pNextList) {
            ...
            if (pReset->command != 'E' && pReset->command != 'G' && pReset->command != 'C')
                break;
...
void
build_delwarn(CHAR_DATA *ch, char *argument)
{
...
void
build_delroom(CHAR_DATA *ch, char *argument)
{
    char                arg1[MAX_INPUT_LENGTH];
    char                arg2[MAX_INPUT_LENGTH];
    ROOM_INDEX_DATA    *pRoomIndex;
    ...
    if (is_number(arg1)) {
        ...
        sendf(ch, "Are you sure you want to delete room [%d] %s?\n\rType delroom ok if you are sure.\n\r", vnum, pRoomIndex->name);
        return;
    }
    ...
    if (pRoomIndex->tele) {
        free_string(pRoomIndex->tele->in);
        free_string(pRoomIndex->tele->out);
        UNLINK(pRoomIndex->tele, first_tele, last_tele, next, prev);
        top_tele--;
        PUT_FREE(pRoomIndex->tele, tele_free);
    }

    if (pRoomIndex->locker) {
        UNLINK(pRoomIndex->locker, first_locker, last_locker, next, prev);
        PUT_FREE(pRoomIndex->tele, tele_free);
    }
...
void
build_delobject(CHAR_DATA *ch, char *argument)
{
    char                arg1[MAX_INPUT_LENGTH];
    char                arg2[MAX_INPUT_LENGTH];
    CHAR_DATA          *vch;
    ...
    if (is_number(arg1)) {
        ...
        sendf(ch, "Are you sure you want to delete object: [%d] %s?\n\rType delobject ok if you are sure.\n\r", vnum, pObjIndex->name);
        return;
    }
...
void
build_delmob(CHAR_DATA *ch, char *argument)
{
    char                arg1[MAX_INPUT_LENGTH];
    char                arg2[MAX_INPUT_LENGTH];
    CHAR_DATA          *vch;
    ...
    if (is_number(arg1)) {
        ...
        sendf(ch, "Are you sure you want to delete mobile: [%d] %s?\n\rType delmobile ok if you are sure.\n\r", vnum, pMobIndex->player_name);
        return;
    }
...
void
build_help(CHAR_DATA *ch, char *argument)
{
    ...
    if (!send_help(ch, arg, HELP_BUILD, FALSE))
...
void
build_helps(CHAR_DATA *ch, char *argument)
{
    send_to_char("@@N@@gBelow is a list of builder helps you can look up using @@ahelp <keyword>@@g while in\n\rthe builder:\n\r\n\r", ch);
    show_helps(ch, HELP_BUILD);
    return;
}
...
void
build_forcereset(CHAR_DATA *ch, char *argument)
{
    AREA_DATA          *pArea;
    ...
    if (str_cmp(argument, "all") || ch->level < MAX_LEVEL)
        reset_area(pArea);
    else
...
void
build_strdup(char **dest, char *src, bool freesrc, CHAR_DATA *ch)
{
...
void
build_editstr(char **dest, char *src, CHAR_DATA *ch)
{
    ...
    write_start(dest, (void *) build_finishedstr, orig, ch);
...
void
build_finishedstr(char *orig, char **dest, CHAR_DATA *ch, bool saved)
{
    if (!saved) {
        *dest = str_dup(orig);
    }
...
void
build_set_oedit(CHAR_DATA *ch, char *argument)
{
    OBJ_INDEX_DATA     *obj;
    ...
    if (is_number(argument)) {
        ...
        if (obj && !build_canread(obj->area, ch, 1))
            return;
...
void
build_set_redit(CHAR_DATA *ch, char *argument)
{
    ch->build_vnum = ch->in_room->vnum;
...
void
build_set_medit(CHAR_DATA *ch, char *argument)
{
    MOB_INDEX_DATA     *mob;
...
void
build_set_mpedit(CHAR_DATA *ch, char *argument)
{
    MOB_INDEX_DATA     *mob;

    ch->act_build = ACT_BUILD_MPEDIT;
    if (is_number(argument)) {
        ch->build_vnum = atoi(argument);
        if ((mob = get_mob_index(ch->build_vnum)) == NULL)
            ch->build_vnum = -1;
...
void
build_set_nedit(CHAR_DATA *ch, char *argument)
{
...
void
build_setvnum(CHAR_DATA *ch, char *argument)
{
    char                buf[MAX_STRING_LENGTH];
...
void
build_list(CHAR_DATA *ch, char *argument)
{
...
void
build_set(CHAR_DATA *ch, char *argument)
{
...
void
build_listvalues(CHAR_DATA *ch, char *argument)
{
    ...
    value *= 10;
    for (foo = 0; foo < 4; foo++)
        sendf(ch, "@@W[Value@@y%d@@W] : @@y%s@@g\n\r", foo, rev_table_lookup(tab_value_meanings, value + foo));

    return;
}

void
build_listweapons(CHAR_DATA *ch, char *argument)
{
    ...
    for (foo = 0; tab_weapon_types[foo].text != NULL; foo++) {
        sendf(ch, "@@W%2ld - @@y%10s.   ", tab_weapon_types[foo].value, tab_weapon_types[foo].text);
...
void
build_listliquids(CHAR_DATA *ch, char *argument)
{
    ...
    for (foo = 0; tab_drink_types[foo].text != NULL; foo++) {
        sendf(ch, "%2ld - %12s.   ", tab_drink_types[foo].value, tab_drink_types[foo].text);
...
void
build_listspells(CHAR_DATA *ch, char *argument)
{
    /* List spells -S- */
    int                 sn;
    int                 type;
    ...
    sendf(ch, "List of spells for option %s:\n\r", argument);
...
void
build_urooms(CHAR_DATA *ch, char *argument)
{
    ...
    sendf(ch, "Room vnum usage summary:\n\r\n\r%s\n\r\n\r%s\n\r", used, free);
    return;
}

void
build_uobjs(CHAR_DATA *ch, char *argument)
{
    /* List vnum usage for area... */
    int                 curvnum;
    char                buf[MAX_STRING_LENGTH];
    ...
    sendf(ch, "Object vnum usage summary:\n\r\n\r%s\n\r\n\r%s\n\r", used, free);
    return;
}

void
build_umobs(CHAR_DATA *ch, char *argument)
{
    /* List vnum usage for area... */
    int                 curvnum;
    char                buf[MAX_STRING_LENGTH];
    ...
    sendf(ch, "Mobile vnum usage summary:\n\r\n\r%s\n\r\n\r%s\n\r", used, free);
    return;
}

void
build_clone(CHAR_DATA *ch, char *argument)
{
    char                arg1[MAX_INPUT_LENGTH];
    char                arg2[MAX_INPUT_LENGTH];
    ...
    if (!str_cmp(arg1, "room")) {
        ...
        if (ch->build_vnum == -1) {
            send_to_char("You must editing a valid room first!\n\r", ch);
            return;
        }
    ...
    if (!str_cmp(arg1, "obj")) {
        ...
        if (ch->build_vnum == -1) {
            send_to_char("You must editing a valid object first!\n\r", ch);
            return;
        }
    ...
    if (!str_cmp(arg1, "mob")) {
        ...
        if (ch->build_vnum == -1) {
            send_to_char("You must editing a valid mobile first!\n\r", ch);
            return;
        }
        ...
        this_mob->hp_mod = mob->hp_mod;
        this_mob->mana_mod = mob->mana_mod;
...
void
mpedit(CHAR_DATA *ch, MPROG_DATA *mprg, int mptype, char *argument)
{
    if (mptype != -1) {
        mprg->type = mptype;
        if (mprg->arglist)
    ...
    ch->act_build = ACT_BUILD_MPEDIT;
    if (!mprg->comlist)
        mprg->comlist = &str_empty[0];
    build_strdup(&mprg->comlist, "$edit", TRUE, ch);
    return;
}

/* 
 * Mobprogram editing - cumbersome-Thoric 
 * Modified From storm's code--       -Aeria 
 */
void
build_mpedit(CHAR_DATA *ch, char *argument)
{
    char                arg1[MAX_INPUT_LENGTH];
    char                arg2[MAX_INPUT_LENGTH];
    char                arg3[MAX_INPUT_LENGTH];
    ...
    if (arg1[0] == '\0' || arg2[0] == '\0') {
        send_to_char("Syntax: \n\r", ch);
        send_to_char(" <command> [number]  [program] [arguments]\n\r", ch);
        send_to_char("\n\r", ch);
        send_to_char("Command being one of:\n\r", ch);
        send_to_char("  addmp delete edit list\n\r", ch);
        send_to_char(" addmp     <program> <arguments>\n\r", ch);
        send_to_char(" edit      <number>  [program] [arguments]\n\r", ch);
        send_to_char(" delete    <number>\n\r", ch);
        send_to_char("\n\r", ch);
        send_to_char("Number is the number of the program on the mob.\n\r", ch);
        send_to_char("Program being one of:\n\r", ch);
        send_to_char("  Type mptypes for a list of values.\n\r", ch);
        return;
    }
    ...
    pArea = pMob->area;
    if (!build_canwrite(pArea, ch, 1)) {
        send_to_char("You do not have permission to edit this area!\n\r", ch);
        return;
    }

    mprog = pMob->first_mprog;
    if (!str_cmp(arg2, "list")) {
        cnt = 0;
        if (!mprog) {
    ...
    if (!str_cmp(arg2, "edit")) {
        ...
        argument = one_argument(argument, arg4);
        if (arg4[0] != '\0') {
            mptype = table_lookup(tab_mprog_types, arg4);
            if (mptype == -1) {
    ...
    if (!str_cmp(arg2, "delete")) {
        ...
        for (mprg = mprog; mprg; mprg = mprg->next) {
            if (++cnt == value) {
                UNLINK(mprg, pMob->first_mprog, pMob->last_mprog, next, prev);
                return;
            }
        }
        return;
    ...
    if (!str_cmp(arg2, "addmp")) {
        mptype = table_lookup(tab_mprog_types, arg3);
        if (mptype == -1 || mptype == 0) {
            send_to_char("Unknown program type.\n\r", ch);
            return;
        }
        GET_FREE(mprg_next, mprog_free);
        mpedit(ch, mprg_next, mptype, argument);
        pMob->progtypes |= mprg_next->type;
        LINK(mprg_next, pMob->first_mprog, pMob->last_mprog, next, prev);
        top_mprog++;
        return;
    }
    build_mpedit(ch, "");
...
void
build_showmobprogs(CHAR_DATA *ch, char *argument)
{
    char                buf1[MAX_STRING_LENGTH];
    char                arg1[MAX_INPUT_LENGTH];
    char                arg2[MAX_INPUT_LENGTH];
    ...
    argument = one_argument(argument, arg2);
    if (arg1[0] == '\0') {
        send_to_char("showmobprogs <vnum>\n\r", ch);
        return;
    }
    if (!is_number(arg1)) {
        send_to_char("must be a vnum.\n\r", ch);
    ...
    if (!pMob->first_mprog) {
        ...
        for (mprg = pMob->first_mprog; mprg != NULL; mprg = mprg->next) {
...
void
build_listmprog_types(CHAR_DATA *ch, char *argument)
{
    /* list mprog types, along with values */
    int                 foo;

    /* Need values as well, so rehash table_printout */
    for (foo = 1; tab_mprog_types[foo].text != NULL; foo++) {
        sendf(ch, "@@y%10s \n\r  ", tab_mprog_types[foo].text);
    }
