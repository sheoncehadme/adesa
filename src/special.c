...
IDSTRING(rcsid, "$Id: special.c,v 1.11 2003/11/15 05:18:20 dave Exp $");
...
DECLARE_SPEC_FUN(spec_del_guard);
DECLARE_SPEC_FUN(spec_hell_guard);
...
spec_lookup(const char *name)
{
    ...
    if (!str_cmp(name, "spec_del_guard"))
        return spec_del_guard;
    if (!str_cmp(name, "spec_hell_guard"))
        return spec_hell_guard;
...
char               *
rev_spec_lookup(void *func)
{
    if (func == (void *) spec_breath_any)
        return "spec_breath_any";
    if (func == (void *) spec_breath_acid)
        return "spec_breath_acid";
    if (func == (void *) spec_breath_fire)
        return "spec_breath_fire";
    if (func == (void *) spec_breath_frost)
        return "spec_breath_frost";
    if (func == (void *) spec_breath_gas)
        return "spec_breath_gas";
    if (func == (void *) spec_breath_lightning)
        return "spec_breath_lightning";
    if (func == (void *) spec_cast_adept)
        return "spec_cast_adept";
    if (func == (void *) spec_cast_cleric)
        return "spec_cast_cleric";
    if (func == (void *) spec_cast_judge)
        return "spec_cast_judge";
    if (func == (void *) spec_cast_mage)
        return "spec_cast_mage";
    if (func == (void *) spec_cast_undead)
        return "spec_cast_undead";
    if (func == (void *) spec_executioner)
        return "spec_executioner";
    if (func == (void *) spec_fido)
        return "spec_fido";
    if (func == (void *) spec_guard)
        return "spec_policeman";
    if (func == (void *) spec_janitor)
        return "spec_janitor";
    if (func == (void *) spec_mayor)
        return "spec_mayor";
    if (func == (void *) spec_poison)
        return "spec_poison";
    if (func == (void *) spec_thief)
        return "spec_thief";
    if (func == (void *) spec_policeman)
        return "spec_policeman";
    if (func == (void *) spec_cast_cadaver)
        return "spec_cast_cadaver";
    if (func == (void *) spec_undead)
        return "spec_undead";
    if (func == (void *) spec_stephen)
        return "spec_stephen";
    if (func == (void *) spec_josh)
        return "spec_josh";
    if (func == (void *) spec_rewield)
        return "spec_rewield";
    if (func == (void *) spec_sylai_priest)
        return "spec_sylai_priest";
    if (func == (void *) spec_cast_bigtime)
        return "spec_cast_bigtime";
    if (func == (void *) spec_sage)
        return "spec_sage";
    if (func == (void *) spec_wizardofoz)
        return "spec_wizardofoz";
    if (func == (void *) spec_mino_guard)
        return "spec_mino_guard";
    if (func == (void *) spec_del_guard)
        return "spec_del_guard";
    if (func == (void *) spec_hell_guard)
        return "spec_hell_guard";
    if (func == (void *) spec_tax_man)
        return "spec_tax_man";
...
print_spec_lookup(char *buf)
{
    ...
    strcat(buf, "       spec_del_guard \n\r");
    strcat(buf, "       spec_hell_guard \n\r");
...
spec_cast_adept(CHAR_DATA *ch)
{
    ...
    switch (number_bits(3)) {
        ...
        case 5:
        {
            ...
            sprintf(buffer, "$n utters the words '%i days till the end of the world as we know it!'", days);
...
spec_executioner(CHAR_DATA *ch)
{
    ...
    if (undead)
        ...
        sprintf(buf, "%s is a %s!  PROTECT THE INNOCENT!  MORE BLOOOOD!!!", victim->short_descr, crime);
...
spec_mino_guard(CHAR_DATA *ch)
{
    ...
    for (victim = ch->in_room->first_person; victim != NULL; victim = victim->next_in_room) {
        ...
        else if (((pass = get_eq_char(victim, WEAR_HOLD)) != NULL)
...
spec_del_guard(CHAR_DATA *ch)
{
    ...
    for (victim = ch->in_room->first_person; victim != NULL; victim = victim->next_in_room) {
        if ((IS_NPC(victim))
            ...
            && (pass->pIndexData->vnum == 1792)) {
            do_say(ch, "Get lost before I change my mind and kill you.");
            continue;
        }
    ...
    do_yell(ch, "An infidel in our midst!  Slay the intruder!");
    multi_hit(ch, victim, TYPE_UNDEFINED);
    return TRUE;
}

bool
spec_hell_guard(CHAR_DATA *ch)
{
    ...
    for (victim = ch->in_room->first_person; victim != NULL; victim = victim->next_in_room) {
        if ((IS_NPC(victim))
            ...
            && (pass->pIndexData->vnum == 6554)) {
            do_say(ch, "@@RThis @@ds@@go@@du@@gl@@R is @@mUntainted! @@eLeave this place o' wretched one!@@N");
            continue;
        }
    ...
    do_yell(ch,
        "@@eT@@Rh@@ei@@Rs @@ds@@go@@du@@gl @@ew@@Ri@@el@@Rl @@em@@Ra@@ek@@Re @@ea @@Rw@@eo@@Rr@@et@@Rh@@ey @@dsacrifice to the @@mDark Lord@@N");
    multi_hit(ch, victim, TYPE_UNDEFINED);
    return TRUE;
}
...
spec_thief(CHAR_DATA *ch)
{
    ...
    for (victim = ch->in_room->first_person; victim != NULL; victim = victim->next_in_room) {
        if (IS_NPC(victim)
            || victim->level >= LEVEL_IMMORTAL || number_bits(2) != 0 || !can_see(ch, victim))    /* Thx Glop */
            continue;
...
spec_policeman(CHAR_DATA *ch)
{
    ...
    sprintf(buf, "%s is a %s, I shall not rest till justice is done!", victim->short_descr, crime);
    do_yell(ch, buf);
...
bool
spec_cast_cadaver(CHAR_DATA *ch)
{
...
bool
spec_undead(CHAR_DATA *ch)
{
    CHAR_DATA          *victim;
    CHAR_DATA          *ach;
    char               *spell;
    int                 sn, sum_lev;
    extern CHAR_DATA   *quest_target, *quest_mob;
    ...
    if (!str_cmp(spell, "summon")) {    /* CHECK FOR NPC!!!!!!!!!!!!! */
        ...
        for (ach = first_char; ach != NULL; ach = ach->next) {
            if (!IS_SET(ach->act, ACT_UNDEAD)
                || ach->level > sum_lev || !IS_NPC(ach)    /* Kavir got summoned!  :P */
                ||ach->in_room == ch->in_room || (ach->in_room && IS_SET(ach->in_room->room_flags, ROOM_NO_RECALL))
                || (ach == quest_mob)
                || (ach == quest_target)
                || !can_see(ch, ach)
                || number_bits(2) != 0)
...
spec_stephen(CHAR_DATA *ch)
{
  ...
  RETRY:
    ...
    act("@@dThe mist settles and $n @@dis gone@@m.@@N", ch, NULL, NULL, TO_ROOM);
    char_from_room(ch);

    char_to_room(ch, room);
    act("@@dA rolling mist brings $n @@dto your side@@m.@@N", ch, NULL, NULL, TO_ROOM);
    ...
    act("$n @@dsays@@m, '@@dThe @@aA@@cn@@Wc@@gi@@We@@gn@@Wt@@gs@@d have sent me to look in on you $N@@m.@@N", ch, NULL, victim, TO_ROOM);

    act("$n @@dswirls the @@am@@ci@@Ws@@gt @@daround $N@@m.@@N", ch, NULL, victim, TO_NOTVICT);
    act("$N @@dlooks at you critically and swirls the @@am@@ci@@Ws@@gt@@d about@@m.@@N", victim, NULL, ch, TO_CHAR);

    act("$n @@dis cloaked in a swirling @@am@@ci@@gs@@Wt@@m.@@N", victim, NULL, ch, TO_NOTVICT);
    act("@@dThe @@am@@ci@@gs@@Wt @@dcloaks you in a healing shroud@@m.@@N", victim, NULL, ch, TO_CHAR);
    sn = skill_lookup("restoration");
    spell_restoration(sn, 666, victim, NULL, NULL);
    return TRUE;
}

/* added by josh for muffin's B-day 10/25 hope she likes it:) */
bool
spec_josh(CHAR_DATA *ch)
{
...
bool
spec_sylai_priest(CHAR_DATA *ch)
{
...
bool
spec_cast_bigtime(CHAR_DATA *ch)
{
    CHAR_DATA          *victim;
    char               *spell;
    int                 sn;
