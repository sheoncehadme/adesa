...
IDSTRING(rcsid, "$Id: ssm.c,v 1.6 2004/06/18 18:21:29 dave Exp $");
...
defrag_heap()
{
    ...
    if (merges)
        xlogf("SSM: defrag_heap: made %d block merges.", merges);
    else
        xlogf("SSM: defrag_heap: resulted in 0 merges.");
...
char               *
_str_dup(const char *str, const char *caller)
{
    BufEntry           *ptr;
    int                 len;
    int                 rlen;
  ...
  RETRY:
    ...
    if (!ptr) {
        ...
        if (numFree >= MAX_FREE) {
            ...
            xlogf("SSM: Attempting to optimize shared string heap.");
            merges = defrag_heap();
 ...
 */
void
_free_string(char *str, const char *caller)
{
    BufEntry           *ptr;
 ...
 */
char               *
_fread_string(FILE * fp, const char *caller)
{
 ...
 */
char               *
_fread_string_eol(FILE * fp, const char *caller)
{
    char                buf[MAX_STRING_LENGTH * 4];
    char               *ptr = buf;
    char                c;
 ...
 */
void
temp_fread_string(FILE * fp, char *buf)
{
    char               *ptr = buf;
    char                c;
