...
IDSTRING(rcsid, "$Id: wizutil.c,v 1.6 2003/08/30 04:59:17 dave Exp $");
...
void
room_pair(ROOM_INDEX_DATA *left, ROOM_INDEX_DATA *right, exit_status ex, char *buffer)
{
    char               *sExit;
...
void
checkexits(ROOM_INDEX_DATA *room, AREA_DATA *pArea, char *buffer)
{
...
void
do_exlist(CHAR_DATA *ch, char *argument)
{
    AREA_DATA          *pArea;
...
do_rename(CHAR_DATA *ch, char *argument)
{
    ...
    if (IS_SET(victim->pcdata->pflags, PFLAG_SPECIALNAME)) {
        send_to_char("Can't rename special named characters.\n\r", ch);
        return;
    }
...

    {
        ...
        for (auc = first_auction; auc != NULL; auc = auc->next) {
            if (!str_cmp(auc->owner, victim->pcdata->origname)) {
                free_string(auc->owner);
                auc->owner = str_dup(capitalize(new_name));
                free_string(auc->owner_name);
                auc->owner_name = str_dup(capitalize(new_name));
            }

            if (!str_cmp(auc->bidder, victim->pcdata->origname)) {
                free_string(auc->bidder);
                auc->bidder = str_dup(capitalize(new_name));
                free_string(auc->bidder_name);
                auc->bidder_name = str_dup(capitalize(new_name));
            }
        }
    }
    ...
    if (victim->adept_level == 20 && (ruler = get_ruler(victim))) {
        ...
        ruler->name = str_dup(capitalize(new_name));
        save_rulers();
    }

    free_string(victim->name);
    victim->name = str_dup(capitalize(new_name));
    if (victim->pcdata->origname)
        free_string(victim->pcdata->origname);
    victim->pcdata->origname = str_dup(capitalize(new_name));
